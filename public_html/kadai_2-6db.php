...
<body>
    ...
    <?php
        ...
        if( empty($_POST['comment']) ) {
            ...
            $clean['view_comment'] = htmlspecialchars( $_POST['comment'], ENT_QUOTES);
        ...
        if( empty($_POST['passcode']) ) {
            ...
            $clean['view_passcode'] = htmlspecialchars( $_POST['passcode'], ENT_QUOTES);
        }


        //新規投稿機能  
        if(empty($error_message)) {
            $dbh = new PDO($dbname, $db_user, $db_pass);
            ...
            //ファイルの存在がある場合は投稿番号+1、なかったら1を指定する
                ...
                $sql = "INSERT INTO message (id, post_num, view_name, view_comment, post_date, view_passcode) VALUES ('$id', '$number', '$clean[view_name]', '$clean[message]', '$date' '$clean[passcode]' ";
        ...
        if(!empty($_POST['deleteNo']) && !empty($_POST['Delpasscode'])){  //DelpasswordがPOSTされていないなら、機能しない
            ...
            foreach($delcons As $delcon){ //ループ処理を行う
                ...
                $id=$deldata[0];
                if(($delete > 0) && (strcmp($Delpassword, $deldata[4]) == 0)){ //削除番号と行番号が一致・不一致
                    if($deldata[0] == $delete){
